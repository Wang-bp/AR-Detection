...
def maskToGraph(mask, ivtslab, quslab, qvslab, costhetas, sinthetas, edge_eps, connectivity=2):
    '''Create graph from AR mask

    Args:
        mask (ndarray): 2D binary map showing the location of an AR with 1s.
        ivtslab (ndarray): 2D IVT or IVT anomaly data.
    ...
    quslab=np.ma.array(quslab)  # these are 0s
    qvslab=np.ma.array(qvslab)  # these are 0s
    wsslab=np.ma.sqrt(quslab**2+qvslab**2)  # these are 0s
...
def getARAxis(g, ivtslab, quslab, qvslab, mask):
    ...
    nodes_arr=np.array(inedgecoor)
    init_dists=spatial.distance_matrix(nodes_arr, nodes_arr)
    upper=np.triu(init_dists)
    selidx=np.where(upper>=np.percentile(init_dists, 95))   #95
    inedgecoor=nodes_arr[np.unique(selidx[0])]
    outedgecoor=nodes_arr[np.unique(selidx[1])]
    inedgecoor=list(zip(inedgecoor[:,0],inedgecoor[:,1]))
    outedgecoor=list(zip(outedgecoor[:,0],outedgecoor[:,1]))
    ...
    if np.max(dists)==0:
        ...
        try:
            ...
            try:
                ...
                g_full=maskToGraph(mask, ivtslab, quslab, qvslab, np.ones(mask.shape),
...
def partPeaks(cropmask, cropidx, orislab, area, min_area, max_ph_ratio,fill_radius):
...
def getARData(slab, quslab, qvslab, anoslab, quano, qvano, areas,lats, lons, mask_list, axis_list, timestr, param_dict):
    ...
    for ii in range(len(mask_list)):
        ...
        cumlen=np.cumsum(lens)
        cumlen=np.insert(cumlen, 0, 0)

        # if fewer than 7 points on axis, interpolate
        if len(axisii)<7:
            if len(axisii)==2:
                axisii=np.linspace(axisii[0], axisii[-1], 7, axis=0)
            else:
                axisii=interpPolygon(axisii, cumlen, 7)
...
def getARDataPolar(slab, quslab, qvslab, anoslab, quano, qvano, areas, lats, lons, mask_list, axis_list, timestr, param_dict):
    '''Fetch AR related data
...
def plotAR(ardf, ax, bmap, lonax):
    ...
    for ii in range(len(ardf)):
        ...
        for xjj, yjj in zip(px_segs, py_segs):
            index = [i for i,x in enumerate(xjj) if x>lonax[-1]]
            if len(index)>0: xjj[index]=lonax[-1]
...
def interpPolygon(coords, t, n):
    '''Interpolate a polygon using B-spline interpolation

    '''
    from scipy.interpolate import splrep,splev

    assert np.ndim(coords)==2, "<coords> needs to be 2D."
    assert coords.shape[1]==2, "<coords> needs to be Nx2."
    assert n>coords.shape[0], "<n> needs to be > len(coords)."

    tnew=np.linspace(t[0], t[-1], n)
    splx=splrep(t, coords[:,0],s=0.,k=1) #don't understand
    x=splev(tnew, splx)
    sply=splrep(t, coords[:,1],s=0.,k=1) #don't understand
    y=splev(tnew, sply)
...
def findARAxis(anoslab, quslab, qvslab, latax, lonax, armask_list, costhetas, sinthetas, param_dict,verbose=True):
    '''Find AR axis
    ...
    for maskii in armask_list:
        #__import__('pdb').set_trace()
        print(i,checkCyclic(maskii))
        i += 1
        '''
        #-------plot--------
        figure=plt.figure(figsize=(14,14),dpi=100)
        m = Basemap(projection='cyl', llcrnrlat=LAT1, urcrnrlat=LAT2, 
                    llcrnrlon=LON1, urcrnrlon=LON2)
        lon_m, lat_m = np.meshgrid(lonax, latax)
        xi, yi = m(lon_m, lat_m)
        # 画平面图
        m.drawparallels(np.arange(0., 90., 10.), linewidth=0,labels=[1,0,0,0], fontsize=10)
        m.drawmeridians(np.arange(-180., 181., 40.), linewidth=0,labels=[0,1,0,1], fontsize=10)
        m.drawcoastlines()
        cs = m.contourf(xi, yi, maskii)
        ...
        gii=maskToGraph(maskii,anoslab,quii,qvii,cosii,sinii,edge_eps)

        #--------------Get AR axis from graph--------------
        axisarrii, axismaskii=getARAxis(gii,anoslab,quii,qvii,maskii)
...
def prepareMeta(lats, lons, times, ntime, nlat, nlon, ref_time='days since 1900-01-01', verbose=True):
...
def prepareMetaPolar(qu, qv, lats, lons, times, ntime, nlat, nlon, dlats, dlons, areamap, ref_time='days since 1900-01-01', verbose=True):
    '''Prepare metadata for AR detection function calls
...
def findARs(ivt, ivtrec, ivtano, qu, qv, lats, lons,
    ...
    for (tidx, timett, label, ardf) in finder_gen:
    ...
    return time_idx, labels_allNV, result_df
...
def findARsGen(ivt, ivtrec, ivtano, qu, qv, lats, lons,
    ...
    for ii, timett in enumerate(timeax):
        ...
        # find AR axis
        axis_list, axismask=findARAxis(slabano, quslab, qvslab, lats, lons, mask_list, costhetas,
        ...
        # fetch AR related data
        labelsNV, ardf=getARData(slab, quslab, qvslab,
